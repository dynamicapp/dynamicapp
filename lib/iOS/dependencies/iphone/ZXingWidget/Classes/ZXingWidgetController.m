...
- (void)decoder:(Decoder *)decoder failedToDecodeImage:(UIImage *)image usingSubset:(UIImage *)subset reason:(NSString *)reason {
    if(!self.isCamera && self.scanImage) {
        [self performSelector:@selector(notifyErrorDelegate:) withObject:reason afterDelay:0.0];
    }
    decoder.delegate = nil;
    [overlayView setPoints:nil];
}

- (void)notifyErrorDelegate:(id)text {
    if (!isStatusBarHidden) [[UIApplication sharedApplication] setStatusBarHidden:NO];
    [delegate zxingController:self didScanFail:text];
}
