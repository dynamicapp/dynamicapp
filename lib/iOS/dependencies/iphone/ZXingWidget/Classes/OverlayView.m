...
- (id) initWithFrame:(CGRect)theFrame cancelEnabled:(BOOL)isCancelEnabled oneDMode:(BOOL)isOneDModeEnabled {
  ...
  if( self ) {
    ...
    if (isCancelEnabled) {
      ...
      [cancelButton setTitle:NSLocalizedString(@"Cancel", @"QR Reader Viewfinder Cancel button") forState:UIControlStateNormal];
      if (oneDMode) {
...
- (void)drawRect:(CGRect)rect {
  ...
  if (displayedMessage == nil) {
    self.displayedMessage = NSLocalizedString(@"Place a barcode inside the viewfinder rectangle to scan it.", @"QR Reader Viewfinder message");
  }
